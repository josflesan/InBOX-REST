...
def test_deliveries_get(api_v1_host, api_key):
...
def test_deliveries_check_hash(api_v1_host, api_key):
...
def test_deliveries_toggle_scanned(api_v1_host, api_key):
    endpoint = os.path.join(api_v1_host, 'deliveries', '63f52275b2422530719ec323')
    response = requests.put(endpoint, headers={"Authorization": f"Bearer {api_key}"})
    assert response.status_code == 201

def test_deliveries_poll_scanned(api_v1_host, api_key):
    endpoint = os.path.join(api_v1_host, 'deliveries', '63f52275b2422530719ec323', 'poll')
    response = requests.get(endpoint, headers={"Authorization": f"Bearer {api_key}"})
    assert response.status_code == 201

def test_deliveries_update_delivered(api_v1_host, api_key):
    endpoint = os.path.join(api_v1_host, 'deliveries', '63f52275b2422530719ec323', 'delivered')
    response = requests.get(endpoint, headers={"Authorization": f"Bearer {api_key}"})
    assert response.status_code == 201
    assert response.json()['result']

def test_deliveries_get_image(api_v1_host, api_key):
...
def test_deliveries_create(api_v1_host, api_key):
    endpoint = os.path.join(api_v1_host, 'deliveries')
    response = requests.post(endpoint, headers={"Authorization": f"Bearer {api_key}"}, json={"hashCode": "acoolhash", "userId": "2"})
    assert response.status_code == 201
